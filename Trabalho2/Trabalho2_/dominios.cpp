...
void Data::validar(string dataRecebida){
    ...
    if(mes == JANEIRO || mes == MARCO || mes == MAIO || mes == JULHO || mes == AGOSTO || mes == OUTUBRO || mes == DEZEMBRO){
        if(dia > DIA_MAXIMO1){
            throw invalid_argument("Argumento de data inv�lido.");
        }
    }
    ...
    for (int i = 0; i < nomeTamanho; i++){
        ...
        if(i == 0){
            if(charAtual < LETRA_A || charAtual > LETRA_Z){
                throw invalid_argument("Argumento de nome inválido.");
            }
        }
        else {
...
* @details Essa fun&ccedil;&atilde;o verifica se a string que ser&aacute; armazenada no atributo moeda atende aos requisitos especificados: assume valores de 0,00 at� 1.000.000,00.
...
void Telefone::validar(string telefoneRecebido){
    ...
    if(telefoneRecebido[0] != '(' || telefoneRecebido[4] != ')' || telefoneRecebido[5] != '-'){
        throw invalid_argument("Argumento de telefone com formato diferente do padrão.");
    }
    for (int i = 1; i < tamanhoTelefone; i++){
        if(i == 4){
            i = 6; //Pula os caracteres especias na checagem de n�mero
        }
        if(telefoneRecebido[i] < '0' || telefoneRecebido[i] > '9'){
            throw invalid_argument("Argumento de telefone com caracter não numérico.");
        }
        if(telefoneRecebido == INVALIDO){
            throw invalid_argument("Argumento (000)-000000000 não é número de telefone.");
        }
    }
}
